/** Dim gothic palette — tuned to lineless overworld sprite refs */
export const P = {
  black: '#000000',
  void: '#0a080c',
  bg: '#121018',
  panel: '#1a161e',
  panel2: '#221c26',
  frame: '#2e2630',
  ink: '#b8aea6',
  inkDim: '#7a7068',
  inkFaint: '#4a4248',
  white: '#d8d4d0',
  bone: '#a09888',

  // Shared chibi skin / vacant eyes
  skin: '#e8dcd0',
  skinShade: '#d0c0b4',
  vacantEye: '#c8d0d8',
  vacantEyeHi: '#e8f0f4',
  crown: '#2a2a30',
  shoe: '#3a3a42',

  // Alice — dusty muted rose-pink
  aliceHair: '#9a6874',
  aliceHairDark: '#6e4852',
  aliceSkin: '#e8dcd0',
  aliceSkinShade: '#d0c0b4',
  aliceCloth: '#8a8088',
  aliceAccent: '#706870',
  aliceCollar: '#d8d4d0',
  aliceEye: '#c8d0d8',

  // Mitzi — pale lavender (matches ref)
  mitziHair: '#9a8ab0',
  mitziHairDark: '#6e6288',
  mitziSkin: '#e8d4c4',
  mitziCloth: '#6a6870',
  mitziAccent: '#a89060',
  mitziSkirt: '#7a6888',
  mitziSkirtDark: '#5a4868',
  mitziPouch: '#4a3858',
  mitziEye: '#d8e0e8',
  mitziCollar: '#e8e4e0',
  mitziShoe: '#e0dcd8',

  // Victoria — dusty maroon (matches ref)
  victoriaHair: '#8a4a58',
  victoriaHairDark: '#5a303c',
  victoriaSkin: '#e8e0d8',
  victoriaCloth: '#2a2a30',
  victoriaClothDark: '#1a1a20',
  victoriaRose: '#6a2830',
  victoriaEye: '#b8c0c8',
  victoriaEyeHi: '#d8e0e8',

  // Hestia — pale bone / white ox
  hestiaHair: '#c8c0b8',
  hestiaHairDark: '#989088',
  hestiaSkin: '#f0e4d8',
  hestiaCloth: '#e8e0d8',
  hestiaClothShade: '#c8c0b8',
  hestiaGold: '#9a8860',
  hestiaEye: '#c8d0d8',
  hestiaEyeAngry: '#a04040',

  // Charon — earth brown
  charonHair: '#6a5040',
  charonHairDark: '#4a3020',
  charonSkin: '#d8c0a8',
  charonCloth: '#3a342e',
  charonAccent: '#8a7050',
  charonEye: '#c8d0d8',

  // Elements
  fire: '#8a4030',
  water: '#3a5a70',
  earth: '#5a6840',
  electric: '#8a7840',

  // UI
  hp: '#6a3038',
  hpFill: '#903848',
  supplies: '#687048',
  mana: '#485868',
  gold: '#8a7050',
  danger: '#903030',
  good: '#406048',

  // Environments
  woods: '#1a2418',
  woodsFar: '#121810',
  volcano: '#2a1814',
  volcanoGlow: '#4a2818',
  fort: '#1a1c20',
  ruins: '#1c1820',
  manor: '#18141c',
  town: '#16141a',
};

export const ELEMENT_CYCLE = {
  water: 'fire',
  fire: 'earth',
  earth: 'electric',
  electric: 'water',
};

export const ELEMENT_WEAK = {
  fire: 'water',
  earth: 'fire',
  electric: 'earth',
  water: 'electric',
};

export const ATTACK_TYPES = ['slashing', 'piercing', 'blunt'];
export const ELEMENTS = ['fire', 'electric', 'earth', 'water'];
